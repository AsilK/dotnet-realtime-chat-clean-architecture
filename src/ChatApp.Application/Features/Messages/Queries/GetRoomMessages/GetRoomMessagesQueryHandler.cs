...
public sealed class GetRoomMessagesQueryHandler : IRequestHandler<GetRoomMessagesQuery, Result<PaginatedList<MessageDto>>>
{
    ...
    {
        var messages = await _unitOfWork.Messages.GetRoomMessagesAsync(request.RoomId, request.PageNumber, request.PageSize, request.BeforeMessageId, cancellationToken);
